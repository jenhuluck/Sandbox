...
def make_focus_zoom_animation_twice(
    ...
    focus_instance_indices = [iidx_focus[i] for i in order]
    bar_labels = [f"inst_{idx}" for idx in focus_instance_indices]
    ...
    if show_distance_bars:
        ...
        ax_bar.set_xlabel(f"Distance to {focus_class} prototype")
        ax_bar.set_title(f"{focus_class} instances only", fontsize=11)
    ...
    def update_bars(alpha, visible):
        ...
        avg_text_before.set_position((avg_before, -0.8))
        avg_text_after.set_position((current_avg, -1.6))
        avg_text_after.set_text(f"current avg={current_avg:.3f}")
